...
<div class="bg-white" x-data="{
    ...
    get estimateTotal() {
        ...
        if (this.selectedSize === 'custom' && this.wFt && this.hFt) {
            ...
            customSqFt = Math.max(1, w * h / 54); // 6x9 = 54 sq ft baseline
        }
        let sizePrice = base * (this.selectedSize === 'custom' ? customSqFt : modifier);
        let add = 0;
            ...
            <div>
                ...
                <div class="flex gap-2 mb-8">
                    ...
                    <button @click="imgIdx = {{ $i }}"
                        ...
                        <img src="{{ asset('storage/'.$img->path) }}" class="w-full h-full object-cover" alt="" loading="lazy">
                    </button>
                    @endforeach
                </div>
                @endif
    ...
    <div x-show="showRoomModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4"
         style="background:rgba(0,0,0,0.5);" @click.self="showRoomModal = false">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden" style="max-height:90vh; overflow-y:auto;">
            <div class="flex items-center justify-between px-5 py-4 border-b" style="border-color:rgba(18,18,18,0.1);">
                ...
                <button @click="showRoomModal = false" class="text-stone-400 hover:text-stone-900">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            @auth
            @if(Auth::user()->ai_credits > 0)
            <form action="{{ route('room.visualize', $product) }}" method="POST" enctype="multipart/form-data" class="p-5 space-y-4">
                @csrf
                <div>
                    ...
                    <div class="flex items-center gap-2 mb-3">
                        <span class="px-2 py-1 rounded text-[10px] font-semibold" style="background:#dcfce7; color:#15803d;">{{ Auth::user()->ai_credits }} credits remaining</span>
                    </div>
                </div>
                <div>
                    ...
                    <div class="relative">
                        <input type="file" name="room_photo" accept="image/*" required
                               ...
                               onchange="document.getElementById('roomFilename').textContent = this.files[0]?.name || 'Choose file...';
                                         document.getElementById('roomPreview').src = URL.createObjectURL(this.files[0]);
